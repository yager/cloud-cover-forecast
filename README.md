...
| 保存した地点を開く | 地図上の水色の丸をタップ（その地点にピンが立つ） |
| 保存した地点を直す・消す | 水色の丸またはピンをタップ → 編集 / 削除 |
| データの説明を見る | 各表の見出しの「ⓘ」 |
...
よく見る場所は地図上に水色の丸として残しておけます。一覧画面はなく、地図上の丸だけが手がかりです。データはこの端末のブラウザ内にだけ保存され、サーバーへ送ることはありません（サイトのデータ削除や別ブラウザでは見えなくなります）。

![保存した地点のメニュー（編集・削除）](icons/ccf-screenshot-pc-places.jpg)

![地点名の編集](icons/ccf-screenshot-pc-places-edit.jpg)
